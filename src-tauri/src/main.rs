...
use crispy_lib::{
    database::{
        get_db_file,
        transactions::{
            delete, get_accounts_by_string, get_total_by_account, get_transaction_details, get_transactions_between, get_transactions_by_account, get_transactions_with_description, insert, update
        },
    }, tree::{build_tree, AccountNode}, types::{Posting, Transaction}
};
...
#[tauri::command]
fn get_all_transactions_between(
    since: Option<Date>,
    until: Option<Date>,
) -> Result<HashMap<i64, Transaction>, String> {
    let conn = open_connection()?;
    get_transactions_between(
        &conn,
        since.unwrap_or(date! {1970 - 1 - 1}),
        until.unwrap_or(Date::MAX),
    )
    .map_err(|e| e.to_string())
}
...
#[tauri::command]
fn search_accounts(input_value: String) -> Result<Vec<String>, String> {
    let conn = open_connection()?;
    get_accounts_by_string(&conn, &input_value).map_err(|e| e.to_string())
}
...
fn get_balance_sheet() -> Result<AccountNode, String> {
    ...
    let all: Vec<(&str, Vec<(&str, Decimal)>)> = assets
        .iter()
        .chain(liabilities.iter())
        .map(|(a, b)| (
            a.as_str(),
            b.iter().filter(|(_,b)| b != &Decimal::ZERO).map(|(c,d)| (c.as_str(), *d)).collect()
        )).collect();
    Ok(build_tree(all))
}
